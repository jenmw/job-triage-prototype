...
/*
  DESIGN DIRECTION: "Honest Utility"
  - Warm cream/paper background, not sterile white
  - Traffic-light colour coding for instant feasibility reads
  - Typographic hierarchy that lets you scan in 3 seconds
  - Accordion progressive disclosure — don't overwhelm
  - Bottom drawer for personalisation (not a modal wall)
  - Tone: direct, empathetic, no corporate fluff
*/
...
const Signal = ({ status, label, detail, subtext }) => {
  const c = status === "good" ? { bg: COLORS.greenBg, border: COLORS.greenBorder, dot: COLORS.green, icon: "✓" }
    : status === "warning" ? { bg: COLORS.amberBg, border: COLORS.amberBorder, dot: COLORS.amber, icon: "~" }
...
const Section = ({ title, icon, children, defaultOpen = false, badge }) => {
  ...
  return (
    <div style={{ borderBottom: `1px solid ${COLORS.border}` }}>
      ...
      <div style={{
        maxHeight: open ? 1200 : 0, overflow: "hidden", transition: "max-height 0.35s ease",
      }}>
...
const AltJob = ({ title, company, pay, rating, location, badge, highlight }) => (
  <div style={{
    padding: "12px 14px", border: `1px solid ${COLORS.border}`, borderRadius: 10, cursor: "pointer",
    transition: "all 0.15s", background: COLORS.card, position: "relative",
  }}
  ...
  return (
    <>
      ...
      <div onClick={onClose} style={{
        position: "fixed", inset: 0, background: "rgba(0,0,0,0.3)", opacity: open ? 1 : 0,
        pointerEvents: open ? "auto" : "none", transition: "opacity 0.3s", zIndex: 100,
      }} />
      {/* Drawer */}
      <div style={{
        position: "fixed", bottom: 0, left: 0, right: 0, maxHeight: "85vh",
        background: COLORS.bg, borderRadius: "20px 20px 0 0", padding: "20px 20px 32px",
        transform: open ? "translateY(0)" : "translateY(100%)", transition: "transform 0.35s ease",
        zIndex: 101, overflowY: "auto", boxShadow: "0 -8px 40px rgba(0,0,0,0.15)",
      }}>
        {/* Handle */}
        ...
        <p style={{ fontSize: 13, color: COLORS.muted, margin: "6px 0 20px", lineHeight: 1.5 }}>
          Answer a few quick ones and we'll show you jobs that actually fit your life. Takes 30 seconds.
        </p>
  ...
  return (
    <div style={{ minHeight: "100vh", background: COLORS.bg, fontFamily: "'Source Sans 3', 'Source Sans Pro', sans-serif", color: COLORS.text }}>
      ...
      <div style={{ maxWidth: 480, margin: "0 auto", padding: "0 16px 120px" }}>
        ...
        {/* ═══ HERO: 3-second scan zone ═══ */}
        <div style={{ padding: "20px 0 16px" }}>
          {/* Employer score badge */}
          <div style={{ display: "flex", alignItems: "center", gap: 10, marginBottom: 12 }}>
            <div style={{
              width: 44, height: 44, borderRadius: 10, background: COLORS.amberBg, border: `1.5px solid ${COLORS.amberBorder}`,
          ...
          <h1 style={{ fontSize: 26, fontWeight: 700, margin: 0, lineHeight: 1.2, fontFamily: "'Fraunces', serif", letterSpacing: -0.5 }}>
            Warehouse Operative
          </h1>
          ...
          <div style={{ display: "flex", gap: 16, marginTop: 12, flexWrap: "wrap" }}>
            {[
              { icon: "💷", text: "£12.58/hr", sub: null },
              { icon: "📍", text: "Hounslow", sub: null },
              { icon: "🕐", text: "Full time", sub: "4 on / 4 off" },
              { icon: "📋", text: "12hr shifts", sub: null },
            ].map((f, i) => (
        ...
        {/* ═══ FEASIBILITY: Can you actually do this job? ═══ */}
        ...
        {/* ═══ QUALITY: What's it actually like? ═══ */}
          ...
          <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: 6 }}>
            {[
              ...
              <div key={v.label} style={{ padding: "8px 10px", borderRadius: 6, background: v.status === "good" ? "#F0FDF4" : "#FEF2F2", fontSize: 11 }}>
                <div style={{ fontWeight: 600, color: v.status === "good" ? COLORS.green : COLORS.red, marginBottom: 3 }}>
                  {v.status === "good" ? "✓" : "✕"} {v.pct}%
                </div>
        ...
        <Section title="What workers actually said" icon="💬">
          ...
          <div style={{ fontSize: 11, color: COLORS.accent, fontWeight: 500, cursor: "pointer", textAlign: "center", padding: "4px 0" }}>
            See all 53 reviews →
          </div>
        </Section>
        ...
        {!personalised && (
          <button onClick={() => setDrawerOpen(true)} style={{
            ...
            border: `1.5px dashed ${COLORS.accent}`, background: COLORS.accentBg,
        ...
        {personalised && (
          <div style={{ marginTop: 16, padding: "12px 16px", background: COLORS.greenBg, borderRadius: 10, border: `1px solid ${COLORS.greenBorder}`, textAlign: "center" }}>
            ...
            <div style={{ fontSize: 11, color: COLORS.muted, marginTop: 2 }}>Based on your commute, pay, and preferences · <span onClick={() => setDrawerOpen(true)} style={{ textDecoration: "underline", cursor: "pointer", color: COLORS.accent }}>Edit</span></div>
          </div>
        )}
      </div>
      ...
      <div style={{
        ...
        <div style={{ maxWidth: 480, margin: "0 auto", display: "flex", gap: 10 }}>
          ...
          <button style={{
            ...
            background: COLORS.card, fontSize: 14, cursor: "pointer", fontFamily: "inherit",
          }}>
            ☆ Save
          </button>
          <button onClick={() => setDrawerOpen(true)} style={{
      ...
      <OnboardingDrawer open={drawerOpen} onClose={() => setDrawerOpen(false)}
        onSubmit={(data) => { setPersonalised(true); setDrawerOpen(false); }} />
    </div>
  );
}
